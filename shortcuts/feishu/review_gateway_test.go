...
func TestReviewGatewayQueuesUnsupportedCommandForVisibleErrorReply(t *testing.T) {
	now := time.Date(2026, 7, 30, 10, 0, 0, 0, time.UTC)
	gateway, err := NewReviewGateway(ReviewGatewayBindings{}, ReviewGatewayConfig{Now: func() time.Time { return now }}, nil)
	if err != nil {
		t.Fatalf("NewReviewGateway: %v", err)
	}
	receipt, err := gateway.Plan(ReviewGatewayEvent{
		MessageID: "om_unknown", EventType: "message", ChatID: "oc_chat", UserID: "ou_user",
		Content: "执行未知操作", CreateTimeMs: now.UnixMilli(),
	})
	if err != nil || !receipt.Accepted || receipt.Job == nil || receipt.Job.Action != "unsupported_command" {
...
func TestReviewGatewayReplyDispatcherRepliesOnceToOriginalMessage(t *testing.T) {
	...
	if !strings.Contains(sender.inputs[0].Text, "owner/repo PR #42") || strings.Contains(sender.inputs[0].Text, "GitLink 写入：0") {
		t.Fatalf("unexpected reply presentation: %s", sender.inputs[0].Text)
	}
	store.mu.Lock()
...
func TestReviewGatewayPRResultUsesInteractiveCard(t *testing.T) {
	job := ReviewGatewayJob{Repository: "owner/repo", PRNumber: 42, ChatID: "chat", SourceMessageID: "message"}
	result := ReviewGatewayExecutionResult{
		Status: "completed",
		ReviewData: &ReviewData{
			Repository: "owner/repo", PullRequest: 42, Title: "Review adapter",
			Author: "alice", State: "open", BaseBranch: "master", HeadBranch: "feature/review",
			HeadSHA: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", GitLinkURL: "https://www.gitlink.org.cn/owner/repo/pulls/42",
			Patchset: ReviewDataPatchset{FilesCount: 2, Additions: 8, Deletions: 3, CommitsCount: 1},
			Summary:  ReviewDataSummary{TotalReviews: 1, ReviewStatus: "approved"},
		},
	}
	input := reviewGatewayResultSendInput(job, result)
	if input.MsgType != "interactive" || input.Card == "" || input.Text != "" {
		t.Fatalf("send input = %#v", input)
	}
	for _, want := range []string{"owner/repo PR #42", "Review adapter", "打开 GitLink PR"} {
		if !strings.Contains(input.Card, want) {
			t.Fatalf("card missing %q: %s", want, input.Card)
		}
	}
}
