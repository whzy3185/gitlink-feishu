...
GitLink CLI 对项目标记（issue_tags）的完整支持：

| 操作 | Shortcut | 说明 |
|------|----------|------|
| 查询标记列表 | `label +list` | 支持 keyword/only_name/sort_by/sort_direction 参数 |
| 创建标记 | `label +create` | 参数：name、description、color |
| 修改标记 | `label +update` | 按 ID 修改，未指定字段会保留 |
| 删除标记 | `label +delete` | 按 ID 删除，支持 `--dry-run` 和 `--yes` |
| 批量创建 | `label +batch-create` | 解析 `name:color:description` 列表，支持 `--dry-run` 和 `--yes` |
| 批量删除 | `label +batch-delete` | 解析逗号分隔 ID，支持 `--dry-run` 和 `--yes` |
