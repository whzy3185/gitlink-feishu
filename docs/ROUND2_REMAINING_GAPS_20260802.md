...
状态：M1 结果卡片代码完成并通过本地门禁；M2 动作代码和真实平台验收仍待完成。

M1 分支已经提供 complete、partial、failed 固定 schema，展示标题、作者、分支、patchset、变更量、Reviewer、线程、风险、unknowns、下一步、负责人和截止日期；公开未绑定仓库只显示只读事实与 GitLink 跳转。卡片超限时会安全降级为文本。尚未推送和部署该分支，因此当前已发布机器人仍可能显示旧文本摘要。
...
M1 部署前群内仍会看到旧文本摘要；M1 部署后可以验收结果卡片，但完整 Review 控制台还需要 M2 的协作动作。
