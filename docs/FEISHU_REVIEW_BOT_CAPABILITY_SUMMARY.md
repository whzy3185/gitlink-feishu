...
所有 PR 级命令必须使用 `查看 owner/repo PR #编号`；仓库之间没有默认优先级。未限定仓库的命令
会要求用户补充完整仓库名，且不会创建 Job。绑定仓库进入完整协作 Job；未绑定仓库只有在 Installation 和群同时启用
...
当前正式输出优先使用 WorkItem 交互卡片，仅在卡片编码或尺寸门禁失败时退回有界文本。每个群内的 PR
维护一条长期卡片：首次创建、内容变化时 PATCH 原卡片、指纹不变时跳过长期更新；同时每次用户查询
都会在当前消息下重新回复完整卡片，不发送无信息量的文本摘要。真实租户中的按钮模板、动作回调和
长期更新仍待验收，不能仅凭 Mock 合同写成平台验收完成。

负责人按 `installation + chat + repository + PR` 隔离。交互卡片使用飞书成员 mention 解析真实姓名，
不再显示写死的“已认领（飞书成员）”；纯文本、Doc 和日志使用已解析名称或不可逆短哈希。
